...
def _qwen35_tool_call_parser(text: str) -> List[Dict]:
    """Extract tool calls from Qwen3.5 output markup.
    ...
    for tool_call_block in tool_call_matches:
        # Extract function name and body from <function=name>...</function>
        function_pattern = r'<function=(\w+)>(.*?)</function>'
        function_matches = re.findall(function_pattern, tool_call_block, re.DOTALL)
...
def _lfm_tool_call_parser(raw_output: str) -> list[dict]:
    """Parse tool calls from LFM model output.

    Expected format:
        <|tool_call_start|>[func(k="v", ...)]<|tool_call_end|>

    Args:
        raw_output: Raw text generated by the model.

    Returns:
        A list of dicts with ``name`` and ``arguments`` keys.
    """
    pattern_block = r"<\|tool_call_start\|>\[(.*?)\]<\|tool_call_end\|>"
    pattern_func  = r"(\w+)\(([^()]*)\)"
    block_match = re.search(pattern_block, raw_output, re.DOTALL)
    tool_calls: list[dict] = []
    if block_match:
        block = block_match.group(1)
        for func_match in re.finditer(pattern_func, block, re.DOTALL):
...
def evaluate_tool_calling_accuracy(
    ...
    if model_type == "qwen3":
        ...
        extract_tool_calls_from_text = _qwen35_tool_call_parser
    elif model_type == "functiongemma":
        extract_tool_calls_from_text = _functiongemma_tool_call_parser
    elif model_type == "lfm2.5":
        extract_tool_calls_from_text = _lfm_tool_call_parser
    else:
        raise ValueError(
            f"Unsupported model type: {model_type}, "
            f"valid types are ['qwen3', 'qwen3.5', 'functiongemma', 'lfm2.5']"
        )
